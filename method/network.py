...
class TriSegNet(nn.Module):
    def __init__(self, in_nc=3, nf=32, out_nc=3, act_type='leakyrelu', threshold=0.05):
        ...
        self.bright_t = 1 - threshold
        self.dark_t = threshold
        self.bright_branch = Restormer(inp_channels=3, out_channels=3, dim=nf // 2, ffn_expansion_factor=0.5)
        self.dark_branch = Restormer(inp_channels=3, out_channels=3, dim=nf // 2, ffn_expansion_factor=0.5)

    def forward(self, x):
        prior = self.prior_downscale(x)
        mid_out = self.mid_branch(x, prior)

        bright_part = torch.clamp((x - self.bright_t) / (1 - self.bright_t), min=0, max=1)
        bright_out = self.bright_branch(bright_part)

        dark_part = torch.clamp((x - self.dark_t) / (0 - self.dark_t), min=0, max=1)
